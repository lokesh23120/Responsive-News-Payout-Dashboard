...
export default function Dashboard() {
  ...
  const filteredNews = news.filter((item) => {
    ...
    const typeMatch = selectedType === "" || selectedType === "news";
    const searchMatch =
      searchQuery === "" ||
  ...
  const handleExportToGoogleSheets = async () => {
    if (!session) return alert("Please log in to export.");
    if (filteredNews.length === 0) return alert("No articles to export.");

    try {
      const topArticles = filteredNews.slice(0, 5);
      for (const article of topArticles) {
        await fetch("/api/add-article", {
          ...
          body: JSON.stringify({
            ...
            author: article.author || session.user?.name || "Unknown",
            date: new Date(article.publishedAt).toISOString().split("T")[0],
          }),
        });
      }
      alert("✅ Articles exported to Google Sheets");
    } catch (error) {
